...
class SweepAutoencoder(nn.Module):
    def __init__(self, input_features, bit_length):
        super().__init__()
        # Compress the sweep down to the desired bit length
...
def build_single_sweep_ids(device_files, reg_index):
    ...
    # --- ML AUTOENCODER TRAINING ---
    X_tensor = torch.tensor(X_scaled, dtype=torch.float32)
    model_ae = SweepAutoencoder(input_features=n_features, bit_length=n_comp)
    criterion = nn.MSELoss()
    optimizer = optim.Adam(model_ae.parameters(), lr=0.001, weight_decay=1e-4)
    
    # Train for 100 epochs
    for epoch in range(100):
        optimizer.zero_grad()
        reconstructed, encoded = model_ae(X_tensor)
        loss = criterion(reconstructed, X_tensor)
        loss.backward()
        optimizer.step()
    
    # Extract the latent representation (projections)
    with torch.no_grad():
        _, Xp_tensor = model_ae(X_tensor)
        Xp = Xp_tensor.numpy()
    
    # Generate binary IDs
    binary_ids = {device_order[i]: binary_from_projection(Xp[i], n_comp) 
                  for i in range(len(device_order))}
                  
    return device_order, binary_ids, {
        "scaler": scaler, 
        "ae_model": model_ae, # Pass the trained PyTorch model instead of PCA
        "ref_freq": REF_FREQ, 
        "use_phase": USE_PHASE, 
        "bit_length": n_comp
    }
...
def build_multi_sweep_ids(device_files, train_indices):
    ...
    # Extract the latent representations (projections)
    with torch.no_grad():
        _, Xp_tensor = model_ae(X_tensor)
        Xp = Xp_tensor.numpy()
        
    # Generate binary IDs by averaging the projections for each device
    binary_ids = {}
    ...
    return binary_ids, {
        "scaler": scaler,
        "ae_model": model_ae, # Pass the PyTorch model instead of PCA
        "ref_freq": REF_FREQ,
        "use_phase": USE_PHASE,
        "bit_length": n_comp
    }
...
def authenticate_files(model, binary_ids, device_files, test_index, threshold=None, excluded=None):
    ...
    Handles both PCA and PyTorch Autoencoder models.
    """
    ...
    for dev, files in sorted(device_files.items()):
        ...
        # --- NEW: Check if using PCA or Autoencoder ---
        if "pca" in model:
            proj = model["pca"].transform(vec_scaled)[0]
        elif "ae_model" in model:
            import torch
            with torch.no_grad():
                vec_tensor = torch.tensor(vec_scaled, dtype=torch.float32)
                _, proj_tensor = model["ae_model"](vec_tensor)
                proj = proj_tensor.numpy()[0]
        else:
            raise KeyError("Model dictionary must contain either 'pca' or 'ae_model'")
        # ----------------------------------------------
...
def run_all_tests(folder):
    ...
    intra_s,
    inter_s,
    "Hamming Distance Distribution (Single-Sweep Autoencoder)",
    ...
    intra_m,
    inter_m,
    "Hamming Distance Distribution (Multi-Sweep supervised learning)",
