...
Single vs Multi Sweep kPCA Authentication
----------------------------------------

Generates IDs using Kernel PCA from device sweep CSVs (like 1_1.csv ... 1_7.csv).
...
KPCA_KERNEL = "rbf"            # Kernel type: 'linear', 'poly', 'rbf', 'sigmoid', 'cosine'
...
def export_device_debug_data(report_dir, single_ids, multi_ids, single_results, multi_results):
    ...
    for both single- and multi-sweep kPCA.
    """
...
def build_single_sweep_ids(device_files, reg_index):
    ...
    kpca = KernelPCA(n_components=n_comp, kernel=KPCA_KERNEL)
    Xp = kpca.fit_transform(scaler.transform(X))
...
def authenticate_files(model, binary_ids, device_files, test_index, threshold=None, excluded=None):
    ...
    scaler, dr_model, bit_length = model["scaler"], model["dr_model"], model["bit_length"]
    results = []
    ...
    for dev, files in sorted(device_files.items()):
        ...
        proj = dr_model.transform(scaler.transform(vec.reshape(1, -1)))[0]
...
def run_all_tests(folder):
    ...
    plot_combined_pdf(
        intra_s,
        inter_s,
        "Hamming Distance Distribution (Single-Sweep kPCA)",
    ...
    plot_combined_pdf(
        intra_m,
        inter_m,
        "Hamming Distance Distribution (Multi-Sweep kPCA)",
